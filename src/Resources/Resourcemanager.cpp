...
ResourceManager::ShaderProgramsMap ResourceManager::m_sharedPrograms;
...
{
	ShaderProgramsMap::const_iterator it = m_sharedPrograms.find(shaderName);
...
{
	std::shared_ptr<RenderEngine::Texture2D> pTexture = loadTexture(std::move(textureName), std::move(texturePath));
	...
	if (pTexture)
	{
		...
		for (const auto& currentSubTextureName : subTextures)
		{
			...
			pTexture->addSubtexture(std::move(currentSubTextureName), std::move(leftBottomUV), std::move(rightTopUV));
...
bool ResourceManager::loadJSONResources(const std::string& JSONpath)
{
	...
	{
		for (const auto& currenShader : shadersIt->value.GetArray())
		{
			const std::string name = currenShader["name"].GetString();
			const std::string filePath_v = currenShader["filePath_v"].GetString();
			const std::string filePath_f = currenShader["filePath_f"].GetString();
	...
	{
		for (const auto& currenTextureAtlas : textureAtlasesIt->value.GetArray())
		{
			const std::string name = currenTextureAtlas["name"].GetString();
			const std::string filePath = currenTextureAtlas["filePath"].GetString();
			const unsigned int width = currenTextureAtlas["width"].GetUint();
			const unsigned int height = currenTextureAtlas["height"].GetUint();
			const unsigned int subTextureWidth = currenTextureAtlas["subTextureWidth"].GetUint();
			const unsigned int subTextureHeight = currenTextureAtlas["subTextureHeight"].GetUint();

			const auto subTexturesArray = currenTextureAtlas["subTextures"].GetArray();
			std::vector<std::string> subTextures;
	...
	if (spritesIt != document.MemberEnd())
	{
		for (const auto& currentSprite : spritesIt->value.GetArray())
		{
			...
			const std::string subTextureName = currentSprite["initalSubTexture"].GetString();

			auto pSprite = loadSprite(name, textureAtlas, shader, subTextureName);
			if (!pSprite)
			{
				std::cerr << "Can't load sprite " << name << std::endl;
				continue;
			}
