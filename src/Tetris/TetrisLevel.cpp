...
void TetrisLevel::checkLine()
{
	for (int i = m_heightBlocks; i > 0; i--)
	{
		...
		int count = std::count_if(m_mapObject.begin(), m_mapObject.end(), [&](IGameObject* obj) {
			return abs(obj->getCurrentPosition().y - figurePositionY) < TETRIS_BLOCK_SIZE / 2; });
		if (count >= 9)
		{
			auto removeIt = std::remove_if(m_mapObject.begin(), m_mapObject.end(), [&](IGameObject* obj) { return abs(obj->getCurrentPosition().y - figurePositionY) < 1; });
			m_mapObject.erase(removeIt, m_mapObject.end());

			for (auto& obj : m_mapObject)
			{
				if (obj->getCurrentPosition().y > figurePositionY)
				{ 
					obj->getCurrentPosition().y = floor((obj->getCurrentPosition().y - TETRIS_BLOCK_SIZE) * TETRIS_BLOCK_SIZE) / TETRIS_BLOCK_SIZE;
				}
			}
		}
	}
